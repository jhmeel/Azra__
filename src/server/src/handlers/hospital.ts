...
export const getHospitalByName = catchAsync(
...
export const getHosptalById = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    const { id } = req.params;
...
export const getAllHospials = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    const response = await databases.listDocuments(
      DATABASE_ID,
      HOSPITAL_COLLECTION_ID
...
export const updateHospitalById = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    const { id } = req.params;
    const {
      email,
...
export const deleteHospitalById = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    const { id } = req.params;
...
export const pingHospital = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (image) {
      const file = base64ToFile(image, `${patient.fullName}.jpg`);
      const fileUpload = await storage.createFile(BUCKET_ID, ID.unique(), file);
    ...
    // Update the "pings" array by pushing the new ping id
    const updatedPings = hospital.pings
      ? [...hospital.pings, JSON.stringify(ping.$id)]
      : [JSON.stringify(ping.$id)];
...
export const updatePing = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    const { error } = updatePingSchema.validate(req.body);
    if (error) return next(new ErrorHandler(400, error.details[0].message));

    const updatedPing = await databases.updateDocument(
      DATABASE_ID,
      PINGS_COLLECTION_ID,
      pingId,
      {
    ...
    res.status(200).json({ success: true, updatedPing });
  }
);
